...
def get_transcript_length(gtf_obj, t_id):

    model = gtf_obj.trans_exons_dt[t_id]
    return model[-1][-1] - model[0][0] + 1


def get_longest_transcript(gtf_obj, t_group):

    return sorted(t_group, key=lambda t_id: (-1 * get_transcript_length(gtf_obj, t_id), t_id))[0]


def get_longest_length(gtf_obj, t_group):

    return get_transcript_length(gtf_obj, get_longest_transcript(gtf_obj, t_group))
...
def sort_by_length(gtf_obj, t_group):

    return sorted(t_group, key=lambda t_id: (get_transcript_length(gtf_obj, t_id), t_id), reverse=True)
...
def group_transcripts_by_consensus(gtf_obj, overlap_group):

    consensus_groups_dt = defaultdict(set)
    for trans_A, trans_B in combinations(overlap_group, 2):

        trans_A_model = get_exon_model(gtf_obj, trans_A)
        trans_B_model = get_exon_model(gtf_obj, trans_B)
    ...
    # Eliminate consensus groups duplicates
    consensus_groups = [sorted(val) for key, val in consensus_groups_dt.items()]
    consensus_groups = [k for k, _ in groupby(sorted(consensus_groups))]

    no_strong_consensus = set(flat(consensus_groups)).symmetric_difference(set(overlap_group))
    for trans_id in no_strong_consensus:
        consensus_groups.append([trans_id])

    # Important! Ensure that transcripts are assigned to only one consensus group
    result, seen = [], set()
    for group in sorted(consensus_groups, key=lambda x: (len(x), x[0]), reverse=True):
        filtered_group = [t_id for t_id in group if t_id not in set(seen)]
        if filtered_group:
            result.append(filtered_group)
        seen.update(filtered_group)

    # Check that all transcripts has been assigned to only one consensus group
    assert len(overlap_group) == len(flat(result))
...
def group_transcripts_across_strands(gtf_obj, to_analyze=None):
    ...
    return across_strand_overlapping_dt


def get_group_boundaries(gtf_obj, t_group):

    trans_id = get_longest_transcript(gtf_obj, t_group)
    trans_exon_boundaries = get_start_end(get_exon_model(gtf_obj, trans_id))

    # To check overlap with mono-exonic transcripts (no introns), we use their exon coordinates instead (force=True)
    trans_intron_boundaries = get_start_end(get_intron_model(gtf_obj, trans_id, force=True))

    return trans_exon_boundaries, trans_intron_boundaries
